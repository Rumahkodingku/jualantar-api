...
final class DeactivateModifier
{
    use ManagesModifierGroups;

    /**
     * Deactivating an active modifier on an active group is refused when it
     * would leave the group unable to satisfy its min selection. On an inactive
     * group it is always allowed. Deactivating a default also resets it.
     */
    public function __invoke(ProductModifierGroup $group, ProductModifier $modifier): Result
    {
        return DB::transaction(function () use ($group, $modifier): Result {
            ...
            $modifier->update(['status' => CatalogStatus::Inactive, 'is_default' => false]);
