...
<head>
    ...
    <title>Perbaikan pengajuan merchant diperlukan</title>
</head>
<body style="margin:0;padding:24px;background:#f5f5f5;font-family:Arial,Helvetica,sans-serif;color:#1f2937;">
    <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;padding:32px;">
        <h1 style="font-size:20px;margin:0 0 16px;">Halo,</h1>
        ...
        <p style="font-size:14px;line-height:1.6;margin:0 0 4px;">Nama usaha: {{ $business_name ?? '-' }}</p>
        @if (! empty($note))
            <p style="font-size:14px;line-height:1.6;margin:0 0 4px;">Catatan: {{ $note }}</p>
        @endif
